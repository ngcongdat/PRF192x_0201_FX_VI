...
(function (todo, data, $) {
	...
	var generateElement = function (params) {
		...
		wrapper.on("dblclick", function () {
			var detail = "Title: " + params.title + "</br>" +
				"Description: " + params.description + "</br>" +
				"Due Date: " + params.date + "</br>" +
				"Owner: " + params.owner;
			generateDialog(detail);
		});

		// Notify invalid date
		wrapper.ready(function () {
			if (params.date != "") {
				var day = params.date.substring(0, 2),
					month = params.date.substring(3, 5) - 1,
					year = params.date.substring(6, 10),
					dateTask = new Date(year, month, day),
					now = new Date();
				if (dateTask < now) {
